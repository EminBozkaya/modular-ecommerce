    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <h1 className="text-3xl font-bold tracking-tight text-gray-900 font-serif">Ürünlerimiz</h1>
            ...
            {isLoading && (
                <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <p className="mt-4 text-gray-500 font-medium font-serif">Ürünler hazırlanıyor...</p>
                </div>
            )}
            ...
            {!isLoading && error && (
                <div className="py-12">
                    <ErrorMessage
                        message={error instanceof Error ? error.message : "Ürünler yüklenirken bir hata oluştu. Lütfen tekrar deneyin."}
                        onRetry={() => refetch()}
                    />
                </div>
            )}
            ...
            {!isLoading && !error && result && Array.isArray(result.items) && result.items.length === 0 && (
                <div className="py-12">
                    <EmptyState
                        ...
                        description="Arama kriterlerinize veya seçtiğiniz kategoriye uygun ürün bulunamadı."
                    />
                </div>
            )}
                            ...
                            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                <div>
                                    <p className="text-sm text-gray-700">
                                        Toplam <span className="font-bold">{result.totalCount}</span> üründen <span className="font-medium">{(page - 1) * pageSize + 1}</span> - <span className="font-medium">{Math.min(page * pageSize, result.totalCount)}</span> arası gösteriliyor
                                    </p>
                                </div>
                                <div>
